...
export function TenantGrid({ tenants }) { return <div className="card-grid">{tenants.map(tenant => <article className="tenant" key={tenant.id}>{tenant.logo_url ? <img src={assetUrl(tenant.logo_url)} alt={`${tenant.name} logo`} loading="lazy" decoding="async"/> : <div className="logo-fallback">{tenant.name[0]}</div>}<div><span>{tenant.location_detail || "Island Central Mactan"}</span><h3>{tenant.name}</h3></div></article>)}</div>; }
